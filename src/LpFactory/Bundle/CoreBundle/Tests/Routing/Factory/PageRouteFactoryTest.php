...
namespace LpFactory\Bundle\CoreBundle\Tests\Routing\Factory;

use LpFactory\Bundle\CoreBundle\Entity\Page;
use LpFactory\Bundle\CoreBundle\Tests\Routing\RoutingHelper;
use LpFactory\Bundle\CoreBundle\Routing\Factory\PageRouteFactory;
use Symfony\Cmf\Bundle\RoutingBundle\Doctrine\Orm\Route;

/**
 * Class PageRouteFactoryTest
 *
 * @package LpFactory\Bundle\CoreBundle\Tests\Routing\Factory
 * @author jobou
 */
class PageRouteFactoryTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $page = $this->getMock('LpFactory\Bundle\CoreBundle\Entity\Page');
        $page
            ->method('getId')
            ->willReturn(56);
    ...
    public function validateEditResult($path, $result)
    {
        ...
        $this->assertEquals('LpFactoryCoreBundle:Page:edit', $defaults['_controller']);
    }
    ...
    public function validateViewResult($path, $result)
    {
        ...
        $defaults = $result->getDefaults();
        $this->assertArrayHasKey('_controller', $defaults);
        $this->assertEquals('LpFactoryCoreBundle:Page:index', $defaults['_controller']);
    }
